...
#define no_of_miles(x)   ((1000/50)*x) //x timer step duration in micros timer
...
uint32_t counter_of_25_micros_for_tripping=0;
volatile uint32_t  counter_of_50_micros_for_general_use=0;
...
{
	   if(htim->Instance ==_50_micro_timer )// IWANT TO STOP ADC INTERRUPT AND TRY TO USE THIS INSTEAD 
	 {
		 // counter_of_25_micros_for_tripping++;
		  counter_of_50_micros_for_general_use++;
		  funcution_time_counter++;
		 
			   if((uint16_t)(counter_of_50_micros_for_general_use%no_of_miles(1))==0)//////////////////////////////////////////////////
  { 
		_1mile_flag=1;
	}
			   if((uint16_t)(counter_of_50_micros_for_general_use%no_of_miles(3))==0)//////////////////////////////////////////////////
  { 
		_3mile_flag=1;
	}
				   if((uint16_t)(counter_of_50_micros_for_general_use%no_of_miles(10))==0)//////////////////////////////////////////////////
  { 
		_10miles_flag=1;
	}	 
	 if(counter_of_50_micros_for_general_use==no_of_miles(500))//40 count equal 1_msec so that this is equal 500_msec
   { 
		 _500miles_flag=1;
		 counter_of_50_micros_for_general_use=0;
	 }
	 }
