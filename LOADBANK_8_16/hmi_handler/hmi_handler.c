...
//	if(((Address>=current_address(0))&&(Address<=current_address(max_no_of_members)))||((Address>=duration_address(0))&&(Address<=duration_address(max_no_of_members))))
	{		for(int i =0;i<max_no_of_members;i++)
	 {
  		if (Address==current_address(i))
	    {
       	current_and_time_values_array[i].current = check_input_current_limits(Value,Address) ;
			...
			else if (Address==duration_address(i))
	    {
				current_and_time_values_array[i].duration = (Value*60) ;
			}
